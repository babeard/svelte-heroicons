...
async function build() {
  ...
  let index = await fs.readFile("src/lib/micro/index.js", { encoding: "utf-8" });
  index = index.replace(/export { (\w+) }/g, "export { $1 as $1Icon }").replace(/from ".\/(\w+)";/g, 'from "./$1/index.js";');
  await fs.writeFile("src/lib/micro/index.js", index);
  ...
  index = await fs.readFile("src/lib/mini/index.js", { encoding: "utf-8" });
  index = index.replace(/export { (\w+) }/g, "export { $1 as $1Icon }").replace(/from ".\/(\w+)";/g, 'from "./$1/index.js";');
